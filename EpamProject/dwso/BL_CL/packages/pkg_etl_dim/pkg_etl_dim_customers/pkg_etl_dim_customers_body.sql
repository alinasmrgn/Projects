CREATE OR REPLACE PACKAGE BODY pkg_etl_dim_customers
AS
    PROCEDURE etl_dim_customers AS
        obj_name VARCHAR(200) := utl_call_stack.concatenate_subprogram(utl_call_stack.subprogram(1));
    BEGIN
        log_writer('MSG', obj_name, sysdate, NULL, 'Start to merge into dim_customers.');
        ...
        log_writer('MSG', obj_name, sysdate, NULL, 'Merged into dim_customers '|| SQL%ROWCOUNT || ' rows.' );            
        COMMIT;
    ...
    END etl_dim_customers;      

END pkg_etl_dim_customers;

-- TRY
exec pkg_etl_dim_customers.etl_dim_customers;